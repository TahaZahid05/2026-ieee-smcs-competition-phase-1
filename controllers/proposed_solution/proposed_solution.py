...
class BasicRosbotController:
    def __init__(self):
        ...
        if self.robot_id == "robot1":
            ...
            target_x, target_y = 2.1, 5.7
        else:
            start_x, start_y = -0.375, 0.0
            target_x, target_y = 4.0, -3.9
    ...
    def set_explore_behavior(self):
        """Navigate to victim using live occupancy grid mapping and A* replanning."""
        current_time = self.robot.getTime()

        # 1. Update live map with current Lidar hits
        self.live_map.update(self.odom.x, self.odom.y, self.odom.heading, self.lidar)

        # 2. Periodic A* replan on the live map
        if (
            not self.navigator.is_arrived
        ...
        # 3. Pure waypoint tracking
        status = self.navigator.step()

        # 4. Periodic position + status log every 5 seconds
        ...
        if current_time - self._last_log_time >= 5.0:
            ...
            print(f"[{self.robot_id}] t={current_time:.0f}s "
                  ...
                  f"mode={status} painted={self.live_map.painted_cells_count} "
                  f"target={wp_str}")
            self._last_log_time = current_time

        # 5. Report victim on arrival
        if status == "arrived" and not self.victim_reported:
            print(f"[{self.robot_id}] Reached target! Reporting to supervisor...")
